...
export default function Signup() {
  const { register, verifyEmailAndLogin, resendVerification } = useAuth()
  const navigate  = useNavigate()
  const location  = useLocation()

  // Support redirect from Login when email needs verification
  const initStep  = location.state?.step  || 'form'
  const initEmail = location.state?.email || ''

  const [step, setStep]         = useState(initStep)   // 'form' | 'verify'
  const [pendingEmail, setPE]   = useState(initEmail)
  ...
  // ── Step 1: submit form → triggers OTP email ───────────────────────────────
  const handleRegister = async (ev) => {
    ...
    try {
      ...
      if (data?.needsVerification) {
        // Account exists but unverified — go to verify step
        setPE(form.email)
        setStep('verify')
        startCountdown()
  ...
  const handlePaste = (ev) => {
    ...
    if (text.length === 6) { setOtp(text.split('')); ev.preventDefault() }
  }

  // ── Step 2: verify OTP → logs user in (NO window.location.reload!) ─────────
  const handleVerify = async (ev) => {
    ...
    setVerify(true)
    try {
      // This stores tokens AND sets user in context atomically
      await verifyEmailAndLogin(pendingEmail, code)
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12 lg:px-12">
        <div className="w-full max-w-md">
          ...
          {step === 'form' && (
            <>
              <div className="mb-8">
                <Link to="/" className="text-sm font-medium mb-6 inline-block" style={{ color: 'var(--muted)' }}>
                  ← Back to home
                </Link>
                <h1 className="font-display text-3xl font-semibold mt-4 mb-1" style={{ color: 'var(--text)' }}>
                  Create Account
                </h1>
              ...
              <form onSubmit={handleRegister} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[['firstName','First Name','Priya'],['lastName','Last Name','Sharma']].map(([k, l, ph]) => (
                    <div key={k}>
                      ...
                      <input
                        className={`input-field text-sm ${errors[k] ? 'error' : ''}`}
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`input-field text-sm ${errors.email ? 'error' : ''}`}
                ...
                <div>
                  <label className="label">
                    Phone{' '}
                    <span className="normal-case tracking-normal font-normal text-xs" style={{ color: 'var(--faint)' }}>
                      (optional)
                    </span>
                  </label>
                  <input
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`input-field text-sm ${errors.confirm ? 'error' : ''}`}
