...
export default function BookSession() {
  ...
  const [selectedSlot,setSel]        = useState(loc.state?.slot || null)
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setAvail(data.session.availability || [])
        if (!selectedSlot) setSel(data.session.availability?.[0] || null)
      } catch { nav('/sessions') }
  ...
  const finalAmount = couponData?.finalAmount ?? session?.price ?? 0
      ...
      <div className="max-w-md w-full mx-auto px-4">
        <div className="card p-10 text-center shadow-card-hover">
          ...
          <div className="rounded-2xl p-5 mb-6 text-left space-y-3"
            ...
            <Row label="Date"     value={new Date(booked.sessionDate).toLocaleDateString('en-IN',{weekday:'long',month:'long',day:'numeric'})} />
            <Row label="Time"     value={booked.sessionTime} />
            {booked.payment?.discountAmount > 0 && <>
              <Row label="Original" value={`₹${booked.payment.originalAmount?.toLocaleString('en-IN')}`} />
              <Row label="Discount" value={<span style={{ color:'var(--primary)' }}>-₹{booked.payment.discountAmount?.toLocaleString('en-IN')}</span>} />
            </>}
            <Row label="Amount"   value={<span className="font-bold text-base" style={{ color:'var(--primary)' }}>₹{booked.payment?.amount?.toLocaleString('en-IN')}</span>} />
          </div>
      ...
      <div className="page-container max-w-5xl">
        ...
        <div className="grid lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="card p-6 shadow-card">
              ...
              <div className="grid sm:grid-cols-2 gap-4 mb-4">
                {[['First Name',user?.firstName],['Last Name',user?.lastName],['Email',user?.email],['Phone',user?.phone||'—']].map(([l,v])=>(
                  <div key={l}><label className="label">{l}</label><input readOnly value={v} className="input-field text-sm"/></div>
                ))}
              </div>
            ...
            {/* Step 3 – Notes + Payment + Coupon */}
            <div className="card p-6 shadow-card">
              <h2 className="font-semibold flex items-center gap-2 mb-5" style={{ color:'var(--text)' }}>
                <Step n="3"/> Notes, Payment &amp; Coupon
              </h2>
              <div className="space-y-4">
                <div>
                  <label className="label">Notes for instructor <Opt/></label>
                  <textarea rows={3} className="input-field resize-none text-sm"
                    placeholder="Any preferences or questions for the instructor…"
                    value={form.notes} onChange={e=>setForm(f=>({...f,notes:e.target.value}))}/>
                </div>
                <div>
                  <label className="label">Special Requirements <Opt/></label>
                  <textarea rows={2} className="input-field resize-none text-sm"
                    placeholder="Injuries, props needed, medical conditions…"
                ...
                <div>
                  ...
                  {couponData ? (
                    ...
                    <div className="flex gap-2">
                      <input className="input-field text-sm flex-1 uppercase"
                      ...
                      <button type="button" onClick={applyCoupon} disabled={couponLoading||!couponCode.trim()}
                        ...
                        {couponLoading ? <span className="spinner w-4 h-4"/> : 'Apply'}
                      </button>
                    </div>
                  )}
                </div>
            ...
            <button onClick={handleBook} disabled={submitting||!selectedSlot}
              ...
              {submitting ? <><span className="spinner w-5 h-5"/>Confirming…</> : 'Confirm Booking'}
            </button>
          </div>
                ...
                {couponData ? (
                  <>
                    <div className="flex justify-between text-sm mb-1">
                      ...
                      <span style={{ color:'var(--muted)', textDecoration:'line-through' }}>₹{session?.price?.toLocaleString('en-IN')}</span>
                    </div>
                    <div className="flex justify-between text-sm mb-2">
                      <span style={{ color:'var(--primary)' }}>Discount</span>
                      <span style={{ color:'var(--primary)' }}>-₹{couponData.discountAmount?.toLocaleString('en-IN')}</span>
                    </div>
                  </>
                ) : null}
...
const Row = ({ label, value }) => (
  <div className="flex justify-between items-center">
    ...
    <span className="font-semibold text-xs text-right" style={{ color:'var(--text)' }}>{value}</span>
  </div>
)
